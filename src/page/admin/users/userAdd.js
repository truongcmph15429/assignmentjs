...
import { add, getAll } from "../../../API/user";
...
const useradd = {
    ...
    print(){
        ...
        <div class="min-h-full">
            ...
            <main>
                ...
                <form class="w-full max-w-4xl m-auto mt-4" method="POST" id="form-add-user">
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-first-name">
                        Username
                    </label>
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="user_name" type="text" placeholder="Jane">
                    </div>
                    <div class="w-full md:w-1/2 px-3">
                    <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">
                        PassWorl
                    </label>
                    <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="password" type="text" placeholder="Doe">
                    </div>
                </div>
                <div class=" -mx-3 mb-2">
    ...
    aftersubmit(){
        ...
        addform.addEventListener("submit",async (Element) => {
            Element.preventDefault();
            const email =document.querySelector("#email").value;
            const {data} = await getAll();
            const checkExist= data.find((data)=>data.email == email);  
            if(checkExist){
